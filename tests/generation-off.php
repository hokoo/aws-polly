...
namespace {
	...
	$meta = array(
		1 => array(
			...
			'itron_polly_tts_audio_link_location' => 'https://example.test/audio.mp3',
		),
	);
	$deletions = array();
	$released = 0;
	...
	function get_post_type( $id ) {
		return get_post( $id )->post_type ?? false; }
	...
	class OfflineCommon extends \iTRON\PollyTTS\Common {
		...
		public function validate_itron_polly_tts_access( $persist_state = true, $show_notices = true ): bool {
			throw new RuntimeException( 'OFF must not validate AWS.' ); }
		public function get_polly_voices( $force_refresh = false ) {
			throw new RuntimeException( 'OFF must not discover voices.' ); }
	...
	$meta[1]['itron_polly_tts_enable'] = '1';
	$snapshot = array(
		'request' => $common->get_audio_voice_request( 1 ),
		'resolved' => $common->get_voice_id(),
	);
	$hash = $common->get_audio_hash( 1, null, $snapshot['resolved'] );
	$assert_current = new ReflectionMethod( $service, 'assert_generation_current' );
	$assert_current->setAccessible( true );
	$assert_current->invoke( $service, 1, $hash, $snapshot );
	check( ! empty( $GLOBALS['cache_deletes'] ), 'In-flight checks invalidate cached inputs before comparison.' );
	foreach ( array( 'global_off', 'post_off', 'text_changed', 'voice_changed', 'post_deleted' ) as $change ) {
		$saved_options = $options;
		$saved_meta = $meta;
		$saved_post = clone $posts[1];
		if ( 'global_off' === $change ) {
			$options['itron_polly_tts_polly_enable'] = ''; }
		if ( 'post_off' === $change ) {
			$meta[1]['itron_polly_tts_enable'] = '0'; }
		if ( 'text_changed' === $change ) {
			$posts[1]->post_content .= ' updated during synthesis'; }
		if ( 'voice_changed' === $change ) {
			$meta[1]['itron_polly_tts_voice_id'] = 'Aditi'; }
		if ( 'post_deleted' === $change ) {
			unset( $posts[1] ); }
		$cancelled = false;
		try {
			$assert_current->invoke( $service, 1, $hash, $snapshot ); } catch ( RuntimeException $e ) {
			$cancelled = true; }
			check( $cancelled, $change . ' prevents publishing a stale generation result.' );
