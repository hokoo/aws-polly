...
if [ ! -L ./wordpress/wp-content/plugins/aws-polly ]; then
  echo -e "${RRED}Creating...${COLOR_OFF}";
  ln -s /srv/web/plugin-dir/ ./wordpress/wp-content/plugins/aws-polly
fi
echo -e "${ICYAN}Result: ${RYELLOW}$(ls -l ./wordpress/wp-content/plugins/ | grep aws-polly)${COLOR_OFF}"
