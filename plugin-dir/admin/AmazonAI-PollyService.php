...
class AmazonAI_PollyService {
	...
	const LOCK_PREFIX              = 'aws-polly-generate-audio-file:';
	...
	public function generate_audio( $post_id ) {
		 ...
		 */
		$continue = true;
		do_action_ref_array( 'amazon_polly_pre_generate_audio', array( $post_id, &$continue, $allowed ) );
		...
		if ( ! $allowed ) {
			...
			do_action( 'amazon_polly_post_generate_audio_locked', $post_id );
			...
			throw new ConcurrentAudioGenerating();
		}
		...
		try {
			if ( $is_polly_enabled ) {
				...
				do_action( 'amazon_polly_post_generate_audio_validated', $post_id );
	...
	*/
	...
	public function convert_to_audio( $post_id, $sample_rate, $voice_id, $sentences, $wp_filesystem, $lang ) {
		...
		if ( $lang == '' ) {
			update_post_meta( $post_id, 'amazon_polly_audio_link_location', $audio_location_link );
			update_post_meta( $post_id, 'amazon_polly_audio_location', $file_handler->get_type() );
			update_post_meta( $post_id, 'amazon_polly_generated_voice_id', $voice_id );
		} else {
			update_post_meta( $post_id, 'amazon_polly_translation_' . $lang, '1' );
		}
